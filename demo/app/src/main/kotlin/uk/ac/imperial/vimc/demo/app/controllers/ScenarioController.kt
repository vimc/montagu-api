...
class ScenarioController(private val db: () -> ScenarioRepository)
{
    ...
    {
        return db().use { it.getScenarios(ScenarioFilterParameters.fromRequest(req)).toList() }
    }

    fun getScenario(req: Request, res: Response): ScenarioAndCoverage
    {
        return db().use { it.getScenarioAndCoverage(req.scenarioId()) }
    }

    fun getCountriesInScenario(req: Request, res: Response): List<Country>
    {
        return db().use { it.getScenarioCountries(req.scenarioId()) }
    }
