...
docker run --rm --entrypoint create_orderly_demo.sh -v "$SRC:/orderly" -u $UID -w /orderly $ORDERLY_IMAGE .

# migrate to add orderlyweb tables
docker pull $OW_MIGRATE_IMAGE
docker run --rm -v "$SRC/demo:/orderly" $OW_MIGRATE_IMAGE

# start orderlyweb
docker pull $ORDERLY_WEB_IMAGE
docker run -d -v "$SRC/demo:/orderly" -p 8888:8888 --net=host --name orderly-web $ORDERLY_WEB_IMAGE
